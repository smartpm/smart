...
#
from smart.backends.arch.loader import ArchDirLoader
from smart.channel import PackageChannel
from smart import *
import os

class ArchDirChannel(PackageChannel):

    def __init__(self, path, recursive, *args):
        super(ArchDirChannel, self).__init__(*args)
        self._path = path
    ...
    def fetch(self, fetcher, progress):
        ...
        if digest == self._digest:
            return True
        self.removeLoaders()
        loader = ArchDirLoader(self._path)
        loader.setChannel(self)
        self._loaders.append(loader)
        if getattr(self, "_recursive", False):
            for root, dirs, files in os.walk(self._path):
                for name in dirs:
                    loader = ArchDirLoader(os.path.join(root, name))
                    loader.setChannel(self)
                    self._loaders.append(loader)
        self._digest = digest
        return True
